...
code_examples:
  css_variables: |
    :root {
      /* Color */
      --color-primary: #1c7dc7;
      --color-primary-hover: #1464a3;
      --color-dark-navy: #0b3558;
      --color-dark-deep: #061e33;
      --color-canvas: #ffffff;
      --color-canvas-ice: #f4f8fc;
      --color-ink: #0f172a;
      --color-ink-muted: #475569;
      --color-ink-inverse: #ffffff;
      --color-hairline: rgba(28, 125, 199, 0.15);
  ...
  button_primary_html_css: |
    <button class="btn-primary">Yêu cầu báo giá</button>
    ...
    .btn-primary {
      ...
      cursor: pointer;
      transition: transform var(--duration-base) var(--ease-overshoot),
    ...
    .btn-primary:hover {
      ...
      box-shadow: 0 10px 25px -5px rgba(12, 59, 102, 0.1), 0 8px 10px -6px rgba(12, 59, 102, 0.05);
    }
  ...
  glass_card_html_css: |
    <div class="card">
      <svg class="card-icon" width="32" height="32" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        <path d="M4 7h16M4 12h16M4 17h10" stroke="var(--color-primary)" stroke-width="1.75" stroke-linecap="round"/>
      </svg>
      <p class="card-eyebrow">Năng lực sản xuất</p>
      <h3 class="card-title">Dây chuyền tự động hóa</h3>
      <p class="card-body">Công suất 12.000 sản phẩm/ngày với kiểm soát chất lượng tự động ở mỗi khâu.</p>
    </div>
    ...
    .card:hover {
      ...
      box-shadow: 0 20px 40px -8px rgba(12, 59, 102, 0.18), 0 10px 16px -6px rgba(12, 59, 102, 0.08);
    }
    .card-eyebrow {
      font-size: 12px; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;
      color: var(--color-primary); margin: 16px 0 8px;
    }
    .card-title { font-size: 20px; font-weight: 700; color: var(--color-ink); margin-bottom: 8px; }
    .card-body { font-size: 15px; line-height: 1.6; color: var(--color-ink-muted); }
    </style>

  hero_overlay_html_css: |
    <section class="hero">
      <img src="/factory-floor.jpg" alt="" class="hero-bg" />
      <div class="hero-overlay"></div>
      <div class="hero-content">
        <span class="badge">Thành lập 1998</span>
        <h1>Sản xuất chính xác.<br/>Tiêu chuẩn toàn cầu.</h1>
        <p>Đối tác chế tạo tin cậy cho hơn 40 thương hiệu công nghiệp.</p>
    ...
    .hero-overlay {
      ...
      background: linear-gradient(to top, rgba(6,30,51,0.9) 0%, rgba(6,30,51,0.4) 50%, rgba(0,0,0,0) 100%);
    }
    .hero-content { position: relative; z-index: 1; padding: 80px; color: #fff; max-width: 720px; }
    .hero-content h1 { font-size: clamp(34px, 4vw + 1rem, 56px); font-weight: 800; line-height: 1.1; letter-spacing: -0.02em; margin: 16px 0; }
    </style>

  gsap_stat_counter_js: |
    // Requires GSAP + ScrollTrigger registered: gsap.registerPlugin(ScrollTrigger)
    document.querySelectorAll('.stat-counter').forEach((el) => {
      ...
      gsap.to(el, {
        ...
        onUpdate: () => { el.textContent = Math.round(el.innerText).toLocaleString(); }
      });
    });

  gsap_card_stagger_js: |
